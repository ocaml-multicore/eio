(** Reading from an empty queue will wait until an item is available. Writing to
    a full queue will wait until there is space.
    ...
    Streams are thread-safe and so can be shared between domains and used to
...
val create : int -> 'a t
(** [create capacity] is a new stream which can hold up to [capacity] items
...
(** [take_nonblocking t] is like [Some (take t)] except that it returns [None]
    if the stream is empty rather than waiting.

    Note that if another domain may add to the stream then a [None] result may
    already be out-of-date by the time this returns. *)
