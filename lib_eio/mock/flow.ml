...
type copy_method = [ `Read_into | `Read_source_buffer ]
...
let make ?(pp = pp_default) label =
  ...
  let copy_rsb_iovec src =
    ...
    traceln "%s: wrote (rsb) @[<v>%a@]" label
      (Fmt.Dump.list (Fmt.using Cstruct.to_string pp))
      bufs;
    len
  in
  let copy_rsb rsb =
  ...
  object (self)
    ...
    method copy src =
      ...
      | `Read_source_buffer ->
          let try_rsb = function
            | Eio.Flow.Read_source_buffer rsb ->
                copy_rsb rsb;
                true
            | _ -> false
          in
    ...
    method set_copy_method m = copy_method := m

    method shutdown cmd =
      traceln "%s: shutdown %s" label
      @@
      match cmd with `Receive -> "receive" | `Send -> "send" | `All -> "all"

    method attach_to_switch sw =
      let hook =
        Switch.on_release_cancellable sw (fun () -> Eio.Flow.close self)
      in
