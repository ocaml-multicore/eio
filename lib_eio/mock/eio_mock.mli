(** Mocks for testing.
    ...
    When testing an Eio program it is often convenient to use mock resources
    rather than real OS-provided ones. This allows precise control over the
    test, such as adding delays or simulated faults. You can always just
    implement the various Eio types directly, but this module provides some
    convenient pre-built mocks, and some helpers for creating your own mocks.

    Mocks typically use {!Eio.traceln} to record how they were used. This output
    can be recorded and compared against a known-good copy using e.g.
    {{:https://github.com/realworldocaml/mdx} ocaml-mdx}.

    Mocks may require configuration. For example, a source flow needs to know
    what data to return when the application reads from it. This can be done
    using the various [on_*] functions. For example:

    {[
      let stdin = Eio_mock.Flow.make "stdin" in
      let stdout = Eio_mock.Flow.make "stdout" in
      Eio_mock.Flow.on_read stdin
        [ `Return "chunk1"; `Return "chunk2"; `Raise End_of_file ];
...
(** Control how a mock responds.
    ...
    This module is mostly useful when writing custom mocks. Individual mocks
    usually provide convenience wrappers around this. *)
module Handler : sig
  type 'a t
  ...
  (** [make default_action] is a new handler that initially always runs
      [default_action]. *)

  val set_handler : 'a t -> (unit -> 'a) -> unit
  (** [set_handler t fn] sets (replaces) the function to be called whenever the
      handler is run. *)

  val seq : 'a t -> 'a actions -> unit
  (** [seq t actions] sets a handler function that performs the next action in
      [actions] on each call. When there are no more actions, it runs the
      default handler. *)
...
module Flow : sig
  ...
  (** [make label] is a mock Eio flow. It can be used as a source, sink, or
      two-way flow.

      @param pp Printer to use to display the data. *)

  val on_read : t -> string Handler.actions -> unit
  (** [on_read t actions] configures the values to return from the mock's [read]
      function. *)

  val on_copy_bytes : t -> int Handler.actions -> unit
  (** [on_copy_bytes t actions] configures the number of bytes to copy in each
      iteration. *)
...
module Net : sig
  type t =
    ...
    ; on_datagram_socket :
        < Eio.Net.datagram_socket ; Eio.Flow.close > Handler.t
  ...
  val on_connect :
    t -> < Eio.Net.stream_socket ; Eio.Flow.close ; .. > Handler.actions -> unit
  (** [on_connect t actions] configures what to do when a client tries to
      connect somewhere. *)

  val on_listen : t -> #Eio.Net.listening_socket Handler.actions -> unit
  (** [on_listen t actions] configures what to do when a server starts listening
  ...
  val on_accept :
    ...
    unit
  (** [on_accept socket actions] configures how to respond when the server calls
      "accept". *)
end

(** {2 Backend for mocks}

    The mocks can be used with any backend, but if you don't need any IO then
    you can use this one to avoid a dependency on eio_main. *)
