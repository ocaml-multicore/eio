...
let add t item =
  ...
  | `Queue_empty ->
      ...
      if Queue.length t.items < t.capacity then (
        ...
        Waiters.await_internal ~mutex:(Some t.mutex) t.writers t.id ctx
          (fun r ->
...
let take t =
  ...
  | None -> (
      ...
      match Waiters.wake_one t.writers () with
      | `Queue_empty -> Waiters.await ~mutex:(Some t.mutex) t.readers t.id
      | `Ok ->
  ...
  | Some v ->
      ...
      (match Waiters.wake_one t.writers () with
      | `Ok (* [length items = t.capacity] again *) | `Queue_empty ->
          () (* [is_empty writers] *));
      Mutex.unlock t.mutex;
      v
