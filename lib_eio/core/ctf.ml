...
module BS = struct
  ...
  external set_64 : Cstruct.buffer -> int -> int64 -> unit
    = "%caml_bigstring_set64"

  external swap64 : int64 -> int64 = "%bswap_int64"
  external unsafe_chr : int -> char = "%identity"

  let set_int8 s off v = Array1.set s off (unsafe_chr v) [@@ocaml.inline]

  let set_int64_le s off v =
    if Sys.big_endian then set_64 s off (swap64 v) else set_64 s off v
    [@@ocaml.inline]
end
...
type hiatus_reason = Wait_for_work | Suspend | Hibernate
...
module Packet = struct
  ...
  *)
  ...
  let set_packet_header_uuid src srcoff dst =
    Cstruct.blit_from_string src srcoff dst 4 16
  ...
  type t = { packet_start : int; header : Cstruct.t; packet_end : int }

  let first_event packet = packet.packet_start + sizeof_packet_header
  let packet_end packet = packet.packet_end
  ...
  let make ~count ~off ~len buffer =
    ...
    let packet = { packet_start = off; header; packet_end = off + len } in
    clear ~count packet;
    packet
end
...
module Control = struct
  ...
  type t = {
    ...
    timestamper : log_buffer -> int -> unit;
        (* Write a timestamp at the given offset. *)
    mutable next_event : int;
        (* Index to write next event (always < packet_end) *)
  ...
  let stop log =
    ...
    | Some active when log == active -> event_log := None
...
*)
  ...
  let write_tid log tid = write64 log (Int64.of_int tid)
...
*)
  ...
  let note_switch log new_current =
    if new_current <> !current_thread then (
      ...
      add_event log op_switch 8 |> write_tid log.log new_current |> end_event)

  let note_suspend log () =
    current_thread := -1;
    add_event log op_switch 8 |> write_tid log.log (-1) |> end_event
...
let note_read ?reader input =
  ...
  | None -> ()
  | Some log ->
      let reader = match reader with None -> !current_thread | Some r -> r in
...
let note_signal ?src dst =
  ...
  | None -> ()
  | Some log ->
      let src = match src with None -> !current_thread | Some x -> x in
