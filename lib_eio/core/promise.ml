...
type 'a state = Resolved of 'a | Unresolved of 'a Waiters.t * Mutex.t
...
let await_exn t = match await t with Ok x -> x | Error ex -> raise ex
...
let peek t =
  match Atomic.get t.state with Unresolved _ -> None | Resolved x -> Some x

let id t = t.id
let is_resolved t = Option.is_some (peek t)
