...
let log_id id name =
  ...
  | None -> ()
  | Some log -> Control.note_label log id name

let log name = log_id !current_thread name
...
let note_created ?label ?loc id ty =
  ...
  | Some log ->
    ...
    Option.iter (Control.note_label log id) loc
...
let note_name = log_id

let note_loc = log_id

let note_parent ~child:_ ~parent:_ = ()
