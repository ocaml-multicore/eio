...
exception
  Multiple of
    exn list (* Note: the last exception in list is the first one reported *)
...
let combine e1 e2 =
  if fst e1 == fst e2 then e1
    ...
    match (e1, e2) with
    | (Cancelled _, _), e | e, (Cancelled _, _) ->
        e
        (* Don't need to report a cancelled exception if we have something better *)
    | (Multiple exs, _), _ when List.memq (fst e2) exs ->
        e1 (* Avoid duplicates *)
