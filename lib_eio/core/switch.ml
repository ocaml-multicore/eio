type t = {
  id : Ctf.id;
  mutable fibers : int;
      (* Total, including daemon_fibers and the main function *)
...
type hook = Null | Hook : Domain.id * 'a Lwt_dllist.node -> hook
...
let remove_hook = function
  | Null -> ()
  | Hook (id, n) ->
      if Domain.self () <> id then
        invalid_arg "Switch hook removed from wrong domain!";
      Lwt_dllist.remove n

let dump f t =
  Fmt.pf f "@[<v2>Switch %d (%d extra fibers):@,%a@]"
    (t.id :> int)
    t.fibers Cancel.dump t.cancel
...
let get_error t = Cancel.get_error t.cancel
let combine_exn ex = function None -> ex | Some ex1 -> Exn.combine ex1 ex

(* Note: raises if [t] is finished or called from wrong domain. *)
let fail ?(bt = Printexc.get_raw_backtrace ()) t ex =
  check_our_domain t;
  if t.exs = None then Ctf.note_resolved t.id ~ex:(Some ex);
  t.exs <- Some (combine_exn (ex, bt) t.exs);
  try Cancel.cancel t.cancel ex
...
let dec_fibers t =
  ...
  if t.fibers = 0 then Waiters.wake_all t.waiter ()

let with_op t fn =
  inc_fibers t;
  Fun.protect fn ~finally:(fun () -> dec_fibers t)
...
let or_raise = function Ok x -> x | Error ex -> raise ex
...
let create cancel =
  ...
  Ctf.note_created id Ctf.Switch;
  {
    id;
    fibers = 1;
    (* The main function counts as a fiber *)
    daemon_fibers = 0;
    exs = None;
...
let run_internal t fn =
  ...
  | v ->
      ...
      Ctf.note_read t.id;
      maybe_raise_exs t;
      (* Check for failure while finishing *)
      (* Success. *)
      v
  | exception ex ->
...
let run_protected fn =
  ...
  Cancel.with_cc ~ctx ~parent:ctx.cancel_context ~protected:true
  @@ fun cancel -> run_internal (create cancel) fn
...
let on_release_full t fn =
  if Domain.self () = t.cancel.domain then
    ...
    | Finished -> (
        ...
        | exception ex ->
            raise (Exn.Multiple [ ex; Invalid_argument "Switch finished!" ]))
  else
    match Cancel.protect fn with
    | () -> invalid_arg "Switch accessed from wrong domain!"
    | exception ex ->
        raise
          (Exn.Multiple
             [ ex; Invalid_argument "Switch accessed from wrong domain!" ])

let on_release t fn = ignore (on_release_full t fn : _ Lwt_dllist.node)
let on_release_cancellable t fn = Hook (t.cancel.domain, on_release_full t fn)
