...
(** {2 Recording events}
    ...
    Libraries and applications can use these functions to make the traces more
    useful. *)
...
(** [should_resolve id] records that [id] is expected to resolve, and should be
    highlighted if it doesn't. *)
...
type event =
  ...
  | Switch
  | Stream
  | Mutex  (** Types of threads or other recorded objects. *)
...
(** [note_try_read src] records that the current thread wants to read from [src]
...
(** [note_resume id] records that the system has resumed (used after
    {!note_hiatus}), and is now running [id]. *)

val note_fork : unit -> id
(** [note_fork ()] records that a new thread has been forked and returns a fresh
    ID for it. *)

val note_resolved : id -> ex:exn option -> unit
(** [note_resolved id ~ex] records that [id] is now resolved. If [ex = None]
    then [id] was successful, otherwise it failed with exception [ex]. *)
...
module Control : sig
  ...
  (** [make ~timestamper b] is a trace buffer that record events in [b]. In most
      cases, the {!Ctf_unix} module provides a simpler interface. *)
