...
let of_flow ?initial_size ~max_size flow =
  ...
  if max_size <= 0 then
    Fmt.invalid_arg "Max size %d should be positive!" max_size;
...
let get t i = Bigarray.Array1.get t.buf (t.pos + i)
...
let peek_char t =
  match ensure t 1 with () -> Some (get t 0) | exception End_of_file -> None
...
let count_while p t =
  let rec aux i =
    if i < t.len then if p (get t i) then aux (i + 1) else i
    else (
      ensure t (t.len + 1);
      aux i)
  in
  try aux 0 with End_of_file -> t.len
...
let skip_while p t =
  let rec aux i =
    if i < t.len then if p (get t i) then aux (i + 1) else consume t i
    else (
      consume t t.len;
      ensure t 1;
      aux 0)
  in
  try aux 0 with End_of_file -> ()

let rec skip n t =
  if n <= t.len then consume t n
  else
    let n = n - t.len in
    consume_all t;
...
let line t =
  ...
  | nl ->
      let len = if nl > 0 && get t (nl - 1) = '\r' then nl - 1 else nl in
      let line =
        Cstruct.to_string (Cstruct.of_bigarray t.buf ~off:t.pos ~len)
      in
      consume t (nl + 1);
      line
...
let at_end_of_input t =
  ...
  else match ensure t 1 with () -> false | exception End_of_file -> true

let end_of_input t =
  if not (at_end_of_input t) then failwith "Unexpected data after parsing"

let pp_pos f t = Fmt.pf f "at offset %d" (consumed_bytes t)
...
let format_errors p t =
  ...
  | exception End_of_file ->
      Fmt.error_msg "Unexpected end-of-file at offset %d" (t.consumed + t.len)
  | exception Buffer_limit_exceeded ->
      Fmt.error_msg "Buffer size limit exceeded when reading %a" pp_pos t
...
let parse_string p s = format_errors (p <* end_of_input) (of_string s)

let parse_string_exn p s =
  match parse_string p s with Ok x -> x | Error (`Msg m) -> failwith m
  [@@inline never]

let bad_offset ~expected actual =
  Fmt.invalid_arg
    "Sequence is stale (expected to be used at offset %d, but stream is now at \
     %d)"
    expected actual
