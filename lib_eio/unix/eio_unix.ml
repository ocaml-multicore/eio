...
let sleep d = Eio.Time.sleep (Effect.perform Private.Get_system_clock) d
...
module FD = struct
  ...
  let as_socket ~sw ~close_unix fd =
    Effect.perform (Private.Socket_of_fd (sw, close_unix, fd))
end

let socketpair ~sw ?(domain = Unix.PF_UNIX) ?(ty = Unix.SOCK_STREAM)
    ?(protocol = 0) () =
...
let getnameinfo (sockaddr : Eio.Net.Sockaddr.t) =
  let sockaddr, options =
    ...
    | `Udp (addr, port) ->
        (Unix.ADDR_INET (Ipaddr.to_unix addr, port), [ Unix.NI_DGRAM ])
  in
  run_in_systhread (fun () ->
      let Unix.{ ni_hostname; ni_service } =
        Unix.getnameinfo sockaddr options
      in
