type t = { waiters : unit Waiters.t; mutex : Mutex.t; id : Ctf.id }

let create () =
  { waiters = Waiters.create (); id = Ctf.mint_id (); mutex = Mutex.create () }
...
let broadcast t = Waiters.wake_all t.waiters ()
