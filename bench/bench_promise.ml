...
type request = { req_body : int; response : response option Promise.u }
and response = { resp_body : int; next_request : request Promise.u }
...
let rec run_server ~n_iters ~i r =
  ...
  assert (req_body = i);
  if i < n_iters then run_server ~n_iters ~i:(succ i) response
  else Promise.resolve response None
...
let run_bench ~domain_mgr ~clock ~use_domains ~n_iters =
  ...
  Fiber.both
    (fun () ->
      ...
      else run_server ~n_iters ~i:0 init_r)
    (fun () -> run_client ~n_iters ~i:0 init_p);
  let t1 = Eio.Time.now clock in
