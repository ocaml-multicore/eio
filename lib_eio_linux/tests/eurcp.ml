...
let cmd =
  ...
  let block_size =
    ...
    Arg.(value & opt int (32 * 1024) & info [ "block-size" ] ~docv:"BYTES" ~doc)
  in
  let queue_depth =
    let doc = "Number of async requests in parallel" in
    Arg.(value & opt int 64 & info [ "queue-depth" ] ~docv:"ENTRIES" ~doc)
  in
  let doc = "copy a file using async effect-based io_uring" in
  let man =
    [ `S "DESCRIPTION"; `P "$(tname) copies a file using Linux io_uring." ]
  in
  let info = Cmd.info "eurcp" ~version:"1.0.0" ~doc ~man in
  Cmd.v info
    Term.(
      const Eurcp_lib.run_cp
      $ block_size
      $ queue_depth
      $ infile
      $ outfile
