...
let test_poll_add () =
  ...
  let sent =
    Unix.write (Eio_linux.FD.to_unix `Peek w) (Bytes.of_string "!") 0 1
  in
  assert (sent = 1);
...
let test_iovec () =
  ...
  Fiber.both
    ...
    (fun () ->
      ...
      Eio_linux.Low_level.writev to_pipe
        [ Cstruct.sub b 3 3; Cstruct.sub b 0 3 ];
      Eio_linux.FD.close to_pipe);
  Alcotest.(check string)
    "Transfer correct" "Got [foo] and [bar]"
    (Cstruct.to_string message)
