...
let src =
  Logs.Src.create "eio_linux" ~doc:"Effect-based IO system for Linux/io-uring"
...
let wrap_errors path fn =
  try fn () with
  | Unix.Unix_error (Unix.EEXIST, _, _) as ex ->
      raise @@ Eio.Fs.Already_exists (path, ex)
  | Unix.Unix_error (Unix.ENOENT, _, _) as ex ->
      raise @@ Eio.Fs.Not_found (path, ex)
  | Unix.Unix_error (Unix.EXDEV, _, _) as ex ->
      raise @@ Eio.Fs.Permission_denied (path, ex)
  | Eio.Fs.Permission_denied _ as ex ->
      raise @@ Eio.Fs.Permission_denied (path, ex)
...
type io_job =
  ...
  | Job : int Suspended.t -> io_job
    (* A negative result indicates error, and may report cancellation *)
  | Write : rw_req -> io_job
...
let wakeup t =
  ...
  match
    ...
    let sent =
      Unix.single_write (FD.get_exn "wakeup" t.eventfd) wake_buffer 0 8
    in
    assert (sent = 8)
  with
...
let cancel job =
  ...
  if res = -2 then
    Log.debug (fun f ->
        f
          "Cancel returned ENOENT - operation completed before cancel took \
           effect")
  else if res = -114 then
    Log.debug (fun f ->
        f
          "Cancel returned EALREADY - operation cancelled while already in \
           progress")
  else if res <> 0 then
...
let with_cancel_hook ~action st fn =
  ...
  | Some ex ->
      enqueue_failed_thread st action ex;
      false
  | None -> (
      match fn () with
      | None -> true
...
let rec submit_rw_req st
    ({ op; file_offset; fd; buf; len; cur_off; action } as req) =
  ...
  | Ok fd ->
      ...
      let retry =
        with_cancel_hook ~action st (fun () ->
            match op with
            | `R -> Uring.read_fixed uring ~file_offset fd ~off ~len (Read req)
            | `W ->
                Uring.write_fixed uring ~file_offset fd ~off ~len (Write req))
      in
      if retry then (
        Ctf.label "await-sqe";
...
let enqueue_read st action (file_offset, fd, buf, len) =
  let file_offset =
    match file_offset with Some x -> x | None -> FD.uring_file_offset fd
  in
...
let rec enqueue_poll_add_unix fd poll_mask st action cb =
  ...
  if retry then
    (* wait until an sqe is available *)
    Queue.push
      (fun st -> enqueue_poll_add_unix fd poll_mask st action cb)
      st.io_q
...
let rec enqueue_splice ~src ~dst ~len st action =
  ...
  | Error ex, _ | _, Error ex -> enqueue_failed_thread st action ex
...
let rec enqueue_openat2 ((access, flags, perm, resolve, dir, path) as args) st
    action =
  ...
  let use fd =
    let retry =
      with_cancel_hook ~action st (fun () ->
          Uring.openat2 st.uring ~access ~flags ~perm ~resolve ?fd path
            (Job action))
    in
    if retry then
      (* wait until an sqe is available *)
...
let rec enqueue_send_msg fd ~fds ~dst buf st action =
  ...
  | Ok unix_fd, Ok unix_fds ->
      ...
      if retry then
        (* wait until an sqe is available *)
        Queue.push
          (fun st -> enqueue_send_msg fd ~fds ~dst buf st action)
          st.io_q
  ...
  | Some IO -> (
      ...
      | (`Wait_until _ | `Nothing) as next_due -> (
          ...
          | None ->
              ...
              in
              Log.debug (fun l ->
                  l "@[<v2>scheduler out of jobs, next timeout %s:@,%a@]"
                    (match timeout with
                    | None -> "inf"
                    | Some v -> string_of_float v)
                    Uring.Stats.pp
                    (Uring.get_debug_stats uring));
...
and handle_complete st ~runnable result =
  ...
  (* If something was waiting for a slot, submit it now. *)
  match runnable with
  | Read req ->
...
and complete_rw_req st ({ len; cur_off; action; _ } as req) res =
  ...
  | e, _ when e < 0 -> (
      ...
      | Some e ->
          Suspended.discontinue action
            e (* If cancelled, report that instead. *)
      | None ->
...
module Low_level = struct
  ...
  let noop () =
    ...
    if result <> 0 then
      raise (Unix.Unix_error (Uring.error_of_errno result, "noop", ""))
  ...
  type _ Effect.t +=
    | ERead :
        (Optint.Int63.t option * FD.t * Uring.Region.chunk * amount)
        -> int Effect.t

  let wrap_connection_failed = function
    | Unix.Unix_error ((ECONNRESET | EPIPE), _, _) as ex ->
        Eio.Net.Connection_reset ex
    | ex -> ex
  ...
  let read_exactly ?file_offset fd buf len =
    ...
    if res < 0 then
      raise
        (wrap_connection_failed
           (Unix.Unix_error (Uring.error_of_errno res, "read_exactly", "")))
  ...
  let read_upto ?file_offset fd buf len =
    ...
    if res < 0 then
      raise
        (wrap_connection_failed
           (Unix.Unix_error (Uring.error_of_errno res, "read_upto", "")))
    else res
  ...
  let readv ?file_offset fd bufs =
    ...
    if res < 0 then
      raise
        (wrap_connection_failed
           (Unix.Unix_error (Uring.error_of_errno res, "readv", "")))
  ...
  let writev_single ?file_offset fd bufs =
    ...
    if res < 0 then
      raise
        (wrap_connection_failed
           (Unix.Unix_error (Uring.error_of_errno res, "writev", "")))
    else res
  ...
  let write ?file_offset fd buf len =
    ...
    if res < 0 then
      raise
        (wrap_connection_failed
           (Unix.Unix_error (Uring.error_of_errno res, "write", "")))
  ...
  let splice src ~dst ~len =
    ...
    if res > 0 then res
      ...
      raise
        (wrap_connection_failed
           (Unix.Unix_error (Uring.error_of_errno res, "splice", "")))
  ...
  let send_msg fd ?(fds = []) ?dst buf =
    ...
    if res < 0 then
      raise
        (wrap_connection_failed
           (Unix.Unix_error (Uring.error_of_errno res, "send_msg", "")))
  ...
  let recv_msg fd buf =
    ...
    if res < 0 then
      raise
        (wrap_connection_failed
           (Unix.Unix_error (Uring.error_of_errno res, "recv_msg", "")));
    (addr, res)
  ...
  let with_chunk ~fallback fn =
    ...
    | Some chunk ->
        Fun.protect ~finally:(fun () -> free_fixed chunk) @@ fun () -> fn chunk
    | None -> fallback ()
  ...
  let openat2 ~sw ?seekable ~access ~flags ~perm ~resolve ?dir path =
    ...
    let res =
      enter (enqueue_openat2 (access, flags, perm, resolve, dir, path))
    in
    ...
    let seekable =
      match seekable with None -> FD.is_seekable fd | Some x -> x
    in
  ...
  let openat ~sw ?seekable ~access ~flags ~perm dir path =
    match dir with
    | FD dir ->
        openat2 ~sw ?seekable ~access ~flags ~perm
          ~resolve:Uring.Resolve.beneath ~dir path
    | Cwd ->
        openat2 ~sw ?seekable ~access ~flags ~perm
          ~resolve:Uring.Resolve.beneath path
    | Fs ->
        openat2 ~sw ?seekable ~access ~flags ~perm ~resolve:Uring.Resolve.empty
          path
  ...
  external eio_mkdirat : Unix.file_descr -> string -> Unix.file_perm -> unit
    = "caml_eio_mkdirat"

  external eio_renameat :
    Unix.file_descr -> string -> Unix.file_descr -> string -> unit
    = "caml_eio_renameat"

  external eio_getrandom : Cstruct.buffer -> int -> int -> int
    = "caml_eio_getrandom"

  external eio_getdents : Unix.file_descr -> string list = "caml_eio_getdents"

  let getrandom { Cstruct.buffer; off; len } = eio_getrandom buffer off len
  ...
  let with_parent_dir dir path fn =
    ...
    Switch.run (fun sw ->
        let parent =
          ...
          | _ ->
              wrap_errors path @@ fun () ->
              openat ~sw ~seekable:false dir dir_path ~access:`R
  ...
  let unlink ~rmdir dir path =
    ...
    if res <> 0 then
      raise @@ Unix.Unix_error (Uring.error_of_errno res, "unlinkat", "")
  ...
  let rename old_dir old_path new_dir new_path =
    ...
    eio_renameat
      (FD.get_exn "renameat-old" old_parent)
      old_leaf
      (FD.get_exn "renameat-new" new_parent)
      new_leaf
  ...
  let open_dir ~sw dir path =
    openat ~sw ~seekable:false dir path ~access:`R
  ...
  let read_dir fd =
    let rec read_all acc fd =
      ...
      | files ->
          let files =
            List.filter (function ".." | "." -> false | _ -> true) files
          in
  ...
  let getaddrinfo ~service node =
    ...
    Unix.getaddrinfo node service [] |> List.filter_map to_eio_sockaddr_t
end
  ...
  object
    ...
    method recv buf =
      ...
      | Unix.ADDR_UNIX _ ->
          raise
            (Failure
               "Expected INET UDP socket address but got Unix domain socket \
                address.")
  end
  ...
  object
    ...
    method accept ~sw =
      ...
      let client_addr =
        ...
        | Unix.ADDR_INET (host, port) ->
            `Tcp (Eio_unix.Ipaddr.of_unix host, port)
      in
...
let net =
  object
    ...
    method listen ~reuse_addr ~reuse_port ~backlog ~sw listen_addr =
      ...
      in
      let sock_unix =
        Unix.socket (socket_domain_of listen_addr) socket_type 0
      in
    ...
    method connect ~sw connect_addr =
      ...
      in
      let sock_unix =
        Unix.socket (socket_domain_of connect_addr) socket_type 0
      in
      let sock = FD.of_unix ~sw ~seekable:false ~close_unix:true sock_unix in
    ...
    method datagram_socket ~sw saddr =
      ...
      | `Udp (host, port) ->
          ...
          let sock =
            FD.of_unix ~sw ~seekable:false ~close_unix:true sock_unix
          in
          Unix.bind sock_unix addr;
          udp_socket sock
    ...
    method run_raw fn =
      ...
      enter (fun t k ->
          domain :=
            Some
              (Domain.spawn (fun () ->
                   Fun.protect fn ~finally:(fun () -> enqueue_thread t k ()))));
...
class dir ~label (fd : dir_fd) =
  object
    inherit Eio.Fs.dir

    method! probe : type a. a Eio.Generic.ty -> a option =
      function Dir_fd -> Some fd | _ -> None

    method open_in ~sw path =
      let fd =
        Low_level.openat ~sw fd path ~access:`R ~flags:Uring.Open_flags.cloexec
          ~perm:0
      in
    ...
    method open_out ~sw ~append ~create path =
      ...
      let fd =
        Low_level.openat ~sw fd path ~access:`RW
    ...
    method open_dir ~sw path =
      let fd =
        Low_level.openat ~sw ~seekable:false fd path ~access:`R
          ~flags:Uring.Open_flags.(cloexec + path + directory)
          ~perm:0
      in
...
let with_uring ~queue_depth ?polling_timeout ?(fallback = no_fallback) fn =
  ...
  | exception Unix.Unix_error (Unix.ENOSYS, _, _) ->
      fallback (`Msg "io_uring is not available on this system")
  | uring -> (
      match fn uring with
      | x ->
          Uring.exit uring;
      ...
      | exception ex ->
          ...
          (try Uring.exit uring
           with ex2 ->
             Log.warn (fun f ->
                 f "Uring.exit failed (%a) while handling another error" Fmt.exn
                   ex2));
...
let rec run :
    ...
    (_ -> a) ->
    a =
 fun ?(queue_depth = 64) ?n_blocks ?(block_size = 4096) ?polling_timeout
     ?fallback main ->
  ...
  let stdenv =
    stdenv
      ~run_event_loop:
        (run ~queue_depth ~n_blocks ~block_size ?polling_timeout ?fallback:None)
  in
  ...
  let mem =
    ...
    | Ok () -> Some (Uring.Region.init ~block_size buf n_blocks)
    | Error `ENOMEM ->
        Log.warn (fun f ->
            f "Failed to allocate %d byte fixed buffer" fixed_buf_len);
        None
  in
          ...
          (fun (type a) (e : a Effect.t) ->
            ...
            | Close fd ->
                Some
                  (fun k ->
                    ...
                    schedule st)
            | Low_level.EWrite args ->
                Some
                  (fun k ->
                    let k = { Suspended.k; fiber } in
                    enqueue_write st k args;
                    schedule st)
            | Low_level.Sleep_until time ->
                Some
                  (fun k ->
                    let k = { Suspended.k; fiber } in
                    match Fiber_context.get_error fiber with
            ...
            | Eio_unix.Private.Await_readable fd ->
                Some
                  (fun k ->
                    ...
                    | None ->
                        let k = { Suspended.k; fiber } in
                        enqueue_poll_add_unix fd
                          Uring.Poll_mask.(pollin + pollerr)
                          st k
                          (fun res ->
                            if res >= 0 then Suspended.continue k ()
                            else
                              Suspended.discontinue k
                                (Unix.Unix_error
                                   ( Uring.error_of_errno res,
                                     "await_readable",
                                     "" )));
                ...
                Some
                  (fun k ->
                    ...
                    | None ->
                        let k = { Suspended.k; fiber } in
                        enqueue_poll_add_unix fd
                          Uring.Poll_mask.(pollout + pollerr)
                          st k
                          (fun res ->
                            if res >= 0 then Suspended.continue k ()
                            else
            ...
            | Eio_unix.Private.Socketpair (sw, domain, ty, protocol) ->
                Some
                  (fun k ->
                    let a, b =
                      Unix.socketpair ~cloexec:true domain ty protocol
                    in
                    let a =
                      FD.of_unix ~sw ~seekable:false ~close_unix:true a |> flow
                    in
                    let b =
                      FD.of_unix ~sw ~seekable:false ~close_unix:true b |> flow
                    in
