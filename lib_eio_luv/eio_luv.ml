...
let () =
  ...
  | Luv_error e ->
      Some
        (Printf.sprintf "Eio_luv.Luv_error(%s) (* %s *)" (Luv.Error.err_name e)
...
let wrap_flow_error e =
  let ex = Luv_error e in
  match e with `ECONNRESET | `EPIPE -> Eio.Net.Connection_reset ex | _ -> ex

let or_raise = function Ok x -> x | Error e -> raise (Luv_error e)
...
type runnable = IO | Thread of (unit -> unit)
...
type t = {
  ...
  mutable fd_map : fd_event_waiters Fd_map.t;
      (* Used for mapping readable/writable poll handles *)
}

type _ Effect.t +=
  | Await :
      (Luv.Loop.t -> Eio.Private.Fiber_context.t -> ('a -> unit) -> unit)
      -> 'a Effect.t

type _ Effect.t += Enter : (t -> 'a Suspended.t -> unit) -> 'a Effect.t

type _ Effect.t +=
  | Enter_unchecked : (t -> 'a Suspended.t -> unit) -> 'a Effect.t
...
module Poll : sig
  ...
  (** [cancel_all t fd] should be called just before [fd] is closed. Any waiters
      will be cancelled. *)
end = struct
  ...
  let enqueue_and_remove t fn (k : unit Suspended.t) v =
    if Fiber_context.clear_cancel_fn k.fiber then fn t k v

  let rec poll_callback t events r =
    (match r with
    | Ok (es : Luv.Poll.Event.t list) ->
        if List.mem `READABLE es then
          apply_all events.read (fun k ->
              enqueue_and_remove t enqueue_thread k ());
        if List.mem `WRITABLE es then
          apply_all events.write (fun k ->
              enqueue_and_remove t enqueue_thread k ())
    | Error e ->
        apply_all events.read (fun k ->
            enqueue_and_remove t enqueue_failed_thread k (Luv_error e));
        apply_all events.write (fun k ->
            enqueue_and_remove t enqueue_failed_thread k (Luv_error e)));
    update t events
  ...
  and update t events =
    ...
    else Luv.Poll.start events.handle m (poll_callback t events)

  let cancel_all t fd =
    match Fd_map.find_opt fd t.fd_map with
    | Some v ->
        let ex = Failure "Closed file descriptor whilst polling" in
        apply_all v.read (fun k ->
            enqueue_and_remove t enqueue_failed_thread k ex);
        apply_all v.write (fun k ->
            enqueue_and_remove t enqueue_failed_thread k ex);
        update t v
    | None -> ()
  ...
  let get_events t fd =
    ...
    | None ->
        let handle =
          Luv.Poll.init ~loop:t.loop (Obj.magic fd : int) |> or_raise
        in
        let events =
          {
            fd;
...
let get_loop () = enter_unchecked @@ fun t k -> Suspended.continue k t.loop
...
module Low_level = struct
  ...
  let await fn = Effect.perform (Await fn)
  let await_exn fn = Effect.perform (Await fn) |> or_raise
  ...
  let await_with_cancel ~request fn =
    enter (fun st k ->
        ...
        Eio.Private.Fiber_context.set_cancel_fn k.fiber (fun ex ->
            ...
            | Error e ->
                Log.debug (fun f ->
                    f "Cancel failed: %s" (Luv.Error.strerror e)));
        fn st.loop (fun v ->
  ...
  module Handle = struct
    ...
    let get op = function
      | { fd = `Open fd; _ } -> fd
      | { fd = `Closed; _ } ->
          invalid_arg (op ^ ": handle used after calling close!")
    ...
    let close t =
      ...
      if t.close_unix then (
        ...
        | Error `EBADF ->
            () (* We don't have a Unix FD yet, so we can't be watching it. *)
  ...
  module File = struct
    ...
    let open_ ~sw ?mode path flags =
      ...
      await_with_cancel ~request (fun loop ->
          Luv.File.open_ ~loop ?mode ~request path flags)
      |> Result.map (of_luv ~sw)

    let read fd bufs =
      let request = Luv.File.Request.make () in
      await_with_cancel ~request (fun loop ->
          Luv.File.read ~loop ~request (get "read" fd) bufs)

    let rec write fd bufs =
      let request = Luv.File.Request.make () in
      let sent =
        await_with_cancel ~request (fun loop ->
            Luv.File.write ~loop ~request (get "write" fd) bufs)
        |> or_raise
      in
    ...
    let realpath path =
      let request = Luv.File.Request.make () in
      await_with_cancel ~request (fun loop ->
          Luv.File.realpath ~loop ~request path)

    let mkdir ~mode path =
      let request = Luv.File.Request.make () in
      await_with_cancel ~request (fun loop ->
          Luv.File.mkdir ~loop ~request ~mode path)

    let unlink path =
      let request = Luv.File.Request.make () in
      await_with_cancel ~request (fun loop ->
          Luv.File.unlink ~loop ~request path)

    let rmdir path =
      let request = Luv.File.Request.make () in
      await_with_cancel ~request (fun loop ->
          Luv.File.rmdir ~loop ~request path)

    let rename old_path new_path =
      let request = Luv.File.Request.make () in
      await_with_cancel ~request (fun loop ->
          Luv.File.rename ~loop ~request old_path ~to_:new_path)

    let opendir path =
      let request = Luv.File.Request.make () in
      await_with_cancel ~request (fun loop ->
          Luv.File.opendir ~loop ~request path)

    let closedir path =
      let request = Luv.File.Request.make () in
      await_with_cancel ~request (fun loop ->
          Luv.File.closedir ~loop ~request path)

    let with_dir_to_read path fn =
      match opendir path with
      | Ok dir ->
          Fun.protect ~finally:(fun () -> closedir dir |> or_raise) @@ fun () ->
          fn dir
      | Error _ as e -> e
    ...
    let readdir path =
      let fn dir =
        ...
        match
          await_with_cancel ~request (fun loop ->
              Luv.File.readdir ~loop ~request dir)
        with
        | Ok dirents ->
            let dirs =
              Array.map (fun v -> v.Luv.File.Dirent.name) dirents
              |> Array.to_list
            in
            Ok dirs
        | Error _ as e -> e
  ...
  module Random = struct
    let rec fill buf =
      ...
      match
        await_with_cancel ~request (fun loop ->
            Luv.Random.random ~loop ~request buf)
      with
      | Ok x -> x
  ...
  module Stream = struct
    ...
    let rec read_into (sock : 'a t) buf =
      let r =
        enter (fun t k ->
            Fiber_context.set_cancel_fn k.fiber (fun ex ->
                Luv.Stream.read_stop (Handle.get "read_into:cancel" sock)
                |> or_raise;
                enqueue_failed_thread t k ex);
            Luv.Stream.read_start
              (Handle.get "read_start" sock)
              ~allocate:(fun _ -> buf)
              (fun r ->
      ...
      | Ok buf' ->
          ...
          if len > 0 then len
            ...
            read_into sock buf (* Luv uses a zero-length read to mean EINTR! *)
    ...
    let of_unix fd = Luv_unix.Os_fd.Socket.from_unix fd |> or_raise
  end
  ...
  let sleep_until due =
    let delay =
      1000. *. (due -. Unix.gettimeofday ()) |> ceil |> truncate |> max 0
    in
    enter @@ fun st k ->
  ...
  let getaddrinfo ~service node =
    ...
    let to_eio_sockaddr_t
        { Luv.DNS.Addr_info.family; addr; socktype; protocol; _ } =
      match (family, socktype) with
      | (`INET | `INET6), (`STREAM | `DGRAM) -> (
          let* host = Luv.Sockaddr.to_string addr in
          let* port = Luv.Sockaddr.port addr in
          let ipaddr =
            Unix.inet_addr_of_string host |> Eio_unix.Ipaddr.of_unix
          in
          match protocol with
    ...
    await_with_cancel ~request (fun loop ->
        Luv.DNS.getaddrinfo ~loop ~request ~service ~node ())
    |> or_raise
...
let socket sock =
  object
    ...
    method shutdown =
      function
      | `Send ->
          await_exn (fun _loop _fiber ->
              Luv.Stream.shutdown (Handle.get "shutdown" sock))
      | (`Receive | `All) as cmd -> (
...
class virtual ['a] listening_socket ~backlog sock =
  object (self)
    ...
    method virtual private make_client : 'a Luv.Stream.t

    method virtual private get_client_addr
        : 'a Stream.t -> Eio.Net.Sockaddr.stream
    ...
    method accept ~sw =
      ...
      match
        Luv.Stream.accept ~server:(Handle.get "accept" sock)
          ~client:(Handle.get "accept" client)
      with
      | Error e ->
          Handle.close client;
          raise (Luv_error e)
...
module Udp = struct
  ...
  let send t buf = function
    | `Udp (host, port) -> (
        ...
        match
          await (fun _loop _fiber ->
              Luv.UDP.send (Handle.get "send" t) bufs
                (luv_addr_of_eio host port))
        with
        | Ok () -> ()
        | Error e -> raise (wrap_flow_error e))
end
...
let listening_ip_socket ~backlog sock =
  object
    ...
    method private get_client_addr c =
      `Tcp
        (Luv.TCP.getpeername (Handle.get "get_client_addr" c)
        |> or_raise
        |> luv_ip_addr_to_eio)
  end

let listening_unix_socket ~backlog sock =
  object
    inherit [[ `Pipe ]] listening_socket ~backlog sock

    method private make_client =
      Luv.Pipe.init ~loop:(get_loop ()) () |> or_raise
...
let net =
  object
    ...
    method listen ~reuse_addr ~reuse_port ~backlog ~sw =
      function
      | `Tcp (host, port) ->
          let sock =
            Luv.TCP.init ~loop:(get_loop ()) () |> or_raise |> Handle.of_luv ~sw
          in
    ...
    method connect ~sw =
      ...
      | `Tcp (host, port) ->
          ...
          let addr = luv_addr_of_eio host port in
          await_exn (fun _loop _fiber ->
              Luv.TCP.connect (Handle.get "connect" sock) addr);
      ...
      | `Unix path ->
          ...
          in
          await_exn (fun _loop _fiber ->
              Luv.Pipe.connect (Handle.get "connect" sock) path);
          (socket sock :> < Eio.Flow.two_way ; Eio.Flow.close >)

    method datagram_socket ~sw =
      function
      | `Udp (host, port) ->
          let domain =
            Eio.Net.Ipaddr.fold ~v4:(fun _ -> `INET) ~v6:(fun _ -> `INET6) host
          in
...
let domain_mgr ~run_event_loop =
  object (self)
    ...
    method run_raw (type a) fn =
      ...
      let async =
        Luv.Async.init ~loop:(get_loop ()) (fun async ->
      ...
      let d =
        Domain.spawn (fun () ->
            result :=
              Some (match fn () with v -> Ok v | exception ex -> Error ex);
...
class dir ~label (dir_path : string) =
  object (self)
    inherit Eio.Fs.dir
    val mutable closed = false

    method! probe : type a. a Eio.Generic.ty -> a option =
      function Dir_resolve_new -> Some self#resolve_new | _ -> None
    ...
    method private resolve ?display_path path =
      if closed then
        Fmt.invalid_arg "Attempt to use closed directory %S" dir_path;
      ...
      if Filename.is_relative path then
        ...
        let full =
          File.realpath (Filename.concat dir_path path) |> or_raise_path path
        in
        let prefix_len = String.length dir_path + 1 in
        if
          String.length full >= prefix_len
          && String.sub full 0 prefix_len = dir_path ^ Filename.dir_sep
        then full
        else if full = dir_path then full
        else
          raise
            (Eio.Fs.Permission_denied
               ( display_path,
                 Failure
                   (Fmt.str "Path %S is outside of sandbox %S" full dir_path) ))
      else
        raise
          (Eio.Fs.Permission_denied
             (display_path, Failure (Fmt.str "Path %S is absolute" path)))
    ...
    method open_in ~sw path =
      let fd =
        File.open_ ~sw (self#resolve path) [ `NOFOLLOW; `RDONLY ]
        |> or_raise_path path
      in
    ...
    method open_out ~sw ~append ~create path =
      ...
      let real_path =
        if create = `Never then self#resolve path else self#resolve_new path
      in
      let fd =
        File.open_ ~sw real_path flags ~mode:[ `NUMERIC mode ]
        |> or_raise_path path
      in
...
let stdenv ~run_event_loop =
  let stdin =
    lazy (source (File.of_luv_no_hook Luv.File.stdin ~close_unix:true))
  in
  let stdout =
    lazy (sink (File.of_luv_no_hook Luv.File.stdout ~close_unix:true))
  in
  let stderr =
    lazy (sink (File.of_luv_no_hook Luv.File.stderr ~close_unix:true))
  in
  object (_ : stdenv)
...
let rec wakeup ~async ~io_queued run_q =
  ...
  | Some IO ->
      ...
      io_queued := false;
      if not (Lf_queue.is_empty run_q) then Luv.Async.send async |> or_raise
  | None -> ()

let rec run : type a. (_ -> a) -> a =
 fun main ->
  ...
  let async =
    Luv.Async.init ~loop (fun async ->
        try wakeup ~async ~io_queued run_q
        with ex ->
          let bt = Printexc.get_raw_backtrace () in
          Fmt.epr "Uncaught exception in run loop:@,%a@." Fmt.exn_backtrace
            (ex, bt);
          Luv.Loop.stop loop)
    |> or_raise
  in
        ...
        effc =
          (fun (type a) (e : a Effect.t) ->
            ...
            | Eio.Private.Effects.Get_context ->
                Some (fun k -> continue k fiber)
            | Enter_unchecked fn -> Some (fun k -> fn st { Suspended.k; fiber })
            | Enter fn ->
                Some
                  (fun k ->
            ...
            | Eio_unix.Private.Await_readable fd ->
                Some
                  (fun k ->
                    match Fiber_context.get_error fiber with
                    | Some e -> discontinue k e
            ...
            | Eio_unix.Private.Await_writable fd ->
                Some
                  (fun k ->
                    match Fiber_context.get_error fiber with
                    | Some e -> discontinue k e
            ...
            | Eio_unix.Private.Get_system_clock ->
                Some (fun k -> continue k clock)
            | Eio_unix.Private.Socket_of_fd (sw, close_unix, fd) ->
                Some
                  (fun k ->
                    try
                    ...
                    with Luv_error _ as ex -> discontinue k ex)
            | Eio_unix.Private.Socketpair (sw, domain, ty, protocol) ->
                Some
                  (fun k ->
                    try
                      if domain <> Unix.PF_UNIX then
                        failwith "Only PF_UNIX sockets are supported by libuv";
                      let ty =
                        match ty with
                        ...
                        | Unix.SOCK_RAW -> `RAW
                        | Unix.SOCK_SEQPACKET ->
                            failwith "Type SEQPACKET not support by libuv"
                      in
  ...
  | `Running ->
      failwith
        "Deadlock detected: no events scheduled but main function hasn't \
         returned"
