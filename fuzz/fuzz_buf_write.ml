...
let op =
  let label (name, gen) =
    Crowbar.with_printer (fun f (Op (s, _)) -> Fmt.pf f "%s:%S" name s) gen
  in
  Crowbar.choose
  @@ List.map label
       [
         ( "string",
           Crowbar.(map [ bytes ]) (fun s -> Op (s, fun t -> W.string t s)) );
         ( "cstruct",
           Crowbar.(map [ cstruct ]) (fun cs ->
               Op (Cstruct.to_string cs, fun t -> W.cstruct t cs)) );
         ( "schedule_cstruct",
           Crowbar.(map [ cstruct ]) (fun cs ->
               Op (Cstruct.to_string cs, fun t -> W.schedule_cstruct t cs)) );
...
let () = Crowbar.(add_test ~name:"random ops" [ list op; bool ] random)
