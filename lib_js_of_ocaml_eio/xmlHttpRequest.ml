  ...
  String.concat "&"
    (List.map
       (function
         ...
         | name, `File s ->
             Url.urlencode name ^ "=" ^ Url.urlencode (to_string s##.name))
       l)
...
let default_response url code headers req =
  {
    ...
    code;
    content =
      Js.Opt.case req##.responseText (fun () -> "") (fun x -> Js.to_string x);
    content_xml =
      (fun () ->
        match Js.Opt.to_option req##.responseXML with
        | None -> None
        | Some doc ->
            if Js.some doc##.documentElement == Js.null then None else Some doc);
    headers;
  }
...
let text_response url code headers req =
  {
    ...
    code;
    content =
      Js.Opt.case req##.responseText (fun () -> Js.string "") (fun x -> x);
...
let perform_raw ?(headers = []) ?content_type ?(get_args = [])
    ?(check_headers = fun _ _ -> true) ?progress ?upload_progress ?contents
    ?override_mime_type ?override_method ?with_credentials (type resptype)
    ~(response_type : resptype response) url =
  ...
  Eio_js_backend.await
    ~setup:(fun ~resolve ~reject ->
      ...
      let do_check_headers =
        ...
        fun () ->
          if !st = `Not_yet then
            if check_headers req##.status headers then st := `Passed
            else (
              reject (Wrong_headers (req##.status, headers));
              st := `Failed;
              req##abort);
          !st <> `Failed
      in
