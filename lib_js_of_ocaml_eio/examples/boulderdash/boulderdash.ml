...
let loading_style =
  js
    "background-color: red; color: white; display:inline; position: absolute; \
     top:0; right:0;"
  ...
  let rec update_cb () =
    ...
    (if not !stopped then
       let str =
         ...
         js
           (Printf.sprintf "%02d:%02d:%02d" (secs / 3600)
              (secs / 60 mod 60)
              (secs mod 60))
       in
...
let rec fall state =
  ...
  for y = Array.length state.map - 2 downto 1 do
    for x = 1 to Array.length state.map.(y) - 2 do
      let sustaining =
        state.map.(y + 1).(x) = Guy && state.map.(y).(x) = Boulder
      in
      if state.map.(y).(x) = Empty && state.map.(y - 1).(x) = Boulder then (
...
let rec build_interaction state show_rem ((_, _, clock_stop) as clock) =
  Eio.Mutex.lock state.map_mutex;
  ...
  let rec update (x, y) next img over_cont out_cont click_cont =
    if walkable state.map.(y).(x) then (
      ...
      := Html.handler (inhibit (with_pending_out (fun () -> ())));
      state.imgs.(y).(x)##.onclick
      := Html.handler (inhibit (with_pending_out click));
  ...
  let update_push ((x, y) as pos) next img img_guy =
    ...
    then (
      ...
      let click () =
        ...
        state.pos <- pos';
        set_cell state x'' y'' Boulder;
        let () = try fall state with Death -> state.dead <- true in
        build_interaction state show_rem clock
      in
      state.imgs.(y').(x')##.onmouseover
      := Html.handler (inhibit (set_pending_out (with_pending_out over) out));
  ...
  if state.pos = state.endpos then (
    ...
    let nil_cont () = () in
    let nil_cont_async () = () in
    update (r state.pos) r (js "sprites/R.png") nil_cont_async nil_cont
      nil_cont_async;
    update (l state.pos) l (js "sprites/L.png") nil_cont_async nil_cont
      nil_cont_async;
    update (u state.pos) u (js "sprites/U.png") nil_cont_async nil_cont
      nil_cont_async;
    update (d state.pos) d (js "sprites/D.png") nil_cont_async nil_cont
      nil_cont_async;
    ...
    show_rem state.rem);
  Eio.Mutex.unlock state.map_mutex
...
let http_get url =
  ...
  if cod = 0 || cod = 200 then msg else assert false

let getfile f = http_get f
...
let start _ =
  ...
  let body =
    Js.Opt.get
      (document##getElementById (js "boulderdash"))
      (fun () -> assert false)
  in
  let board_div = Html.createDiv document in
  let ((clock_div, clock_start, _) as clock) = clock_div ~sw in
  let load_data name process =
    let loading_end = loading body in
    let data = getfile name in
    ...
    load_data "maps.txt" (fun txt ->
        let find_string st =
          ...
          let rec find_string_start s =
            if s >= sz then raise Eos
            else if txt.[s] == '"' then find_string_end (s + 1) (s + 2)
            else find_string_start (s + 1)
          and find_string_end s e =
            if s >= sz then raise Eos
            else if txt.[e] == '"' then (String.sub txt s (e - s), e + 1)
  ...
  let load_level file =
    load_data file (fun data ->
        ...
        let style =
          js
            "border-collapse:collapse;line-height: 0; opacity: 0; \
  ...
  select##.onchange :=
    Html.handler (fun _ ->
        ...
        if i >= 0 && i < List.length levels then
          Eio_js.start (fun () -> load_level (fst (List.nth levels i)));
        Js._false);
